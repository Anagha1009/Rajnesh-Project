...
public partial class City_Colleges : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            ltl_bredcrumbs.Text = "<a href='https://www.eduvidya.com/'>Home</a>Search Colleges";
    ...
    protected void fn_BindColleges()
    {
        try
        {
            ...
            if (Request.QueryString["Category"] != null && Request.QueryString["CategoryID"] != null)
            {
                ...
                bStatus = true;
                strQuery += " WHERE Institute_ID IN(SELECT instituteCategoryList_instituteID FROM edu_InstituteCategoryList WHERE instituteCategoryList_catID=" + iCategoryID + ")";
            }
            ...
            if (Request.QueryString["City"] != null && Request.QueryString["CityID"] != null)
            {
                ...
                if (bStatus == true)
                {
                    strQuery += " AND Institute_CityID=" + iCityID;
                }
                else
                {
                    strQuery += " WHERE Institute_CityID=" + iCityID;
                }
            }

            strQuery += " ORDER BY Institute_Title ASC";

            InstituteClass objInstitute = new InstituteClass();
            CoreWebList<InstituteClass> objInstituteList = objInstitute.fn_SearchByQuery(strQuery);
