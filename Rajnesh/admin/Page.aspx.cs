...
public partial class admin_Page : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
			...
			{
				Response.Redirect(VirtualPathUtility.ToAbsolute("~/admin/Category.aspx"), true);
			}

            HtmlGenericControl BredCrumbs = (HtmlGenericControl)Master.FindControl("BredCrumbs");
            BredCrumbs.InnerHtml = "<a class='link' href='" + VirtualPathUtility.ToAbsolute("~/admin/") + "'>Home</a> &nbsp; > &nbsp; &nbsp;<a class='link' href='" + VirtualPathUtility.ToAbsolute("~/admin/InstituteCategory.aspx") + "'>Institute Category</a> &nbsp; > &nbsp; &nbsp;Page";
    ...
    protected void fn_BindRecords()
    {
        try
        {
			...
			objPage.iCategoryID = iCategoryID;
			CoreWebList<PageClass> objPageList = objPage.fn_getPageByCategoryID();
			if (objPageList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objPageList;
				grd_Records.DataSource = objPageList;
			}
			else
			{
    ...
    protected void grd_Records_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        {
            PageClass objPage = new PageClass();
			objPage.iID = int.Parse(grd_Records.DataKeys[e.RowIndex].Value.ToString());

			string strResponse = objPage.fn_deletePage();
    ...
    protected void dv_Records_ItemInserting(object sender, DetailsViewInsertEventArgs e)
    {
        try
        {
			...
			PageClass objPage = new PageClass();
			objPage.iCategoryID = iCategoryID;
			objPage.strTitle = txtTitle.Text;
            objPage.strDetails = txtDetails.Text;

			string strResponse = objPage.fn_savePage();
    ...
    protected void dv_Records_ItemUpdating(object sender, DetailsViewUpdateEventArgs e)
    {
        try
        {
			...
			PageClass objPage = new PageClass();
			objPage.iID = int.Parse(dv_Records.DataKey.Value.ToString());
			objPage.strTitle = txtTitle.Text;
            objPage.strDetails = txtDetails.Text;

			string strResponse = objPage.fn_editPage();
    ...
    protected void btnSearch_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            PageClass objPage = new PageClass();
			objPage.strTitle = txtSearch.Text.Trim();
			CoreWebList<PageClass> objPageList = objPage.fn_getPageByKeys();
			if (objPageList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objPageList;
				grd_Records.DataSource = objPageList;
