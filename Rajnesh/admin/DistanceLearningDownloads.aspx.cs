...
public partial class admin_DistanceLearningDownloads : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                if (Request.QueryString["DLID"] != null)
                {
                    ...
                    objDwn.strType = "DistanceLearning";
                    objDwn.iTypeID = int.Parse(Request.QueryString["DLID"].ToString());
    ...
    protected void dv_Downloads_ItemInserting(object sender, DetailsViewInsertEventArgs e)
    {
        try
        {
            ...
            if (fuFile.HasFile)
            {
                ...
                strDocPath = Server.MapPath("~/admin/Downloads/DistanceLearning/" + strRanFileName);
