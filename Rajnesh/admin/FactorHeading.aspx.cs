...
public partial class admin_FactorHeading : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            BredCrumbs.InnerHtml = "<a class='link' href='" + VirtualPathUtility.ToAbsolute("~/admin/") + "'>Home</a> &nbsp; > &nbsp; &nbsp;FactorHeading";
    ...
    protected void fn_BindRecords()
    {
        try
        {
            FactorHeadingClass objFactorHeading = new FactorHeadingClass();
			CoreWebList<FactorHeadingClass> objFactorHeadingList = objFactorHeading.fn_getFactorHeadingList();
			if (objFactorHeadingList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objFactorHeadingList;
				grd_Records.DataSource = objFactorHeadingList;
			}
			else
			{
    ...
    protected void fn_BindDetails(int iRecordID)
    {
        ...
        {
            FactorHeadingClass objFactorHeading = new FactorHeadingClass();
			objFactorHeading.iID = iRecordID;
			CoreWebList<FactorHeadingClass> objFactorHeadingList = objFactorHeading.fn_getFactorHeadingByID();
			if (objFactorHeadingList.Count > 0)
			{
				dv_Records.DataSource = objFactorHeadingList;
				dv_Records.DataBind();
			}
        }
        catch (Exception ex)
    ...
    protected void grd_Records_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        {
            FactorHeadingClass objFactorHeading = new FactorHeadingClass();
			objFactorHeading.iID = int.Parse(grd_Records.DataKeys[e.RowIndex].Value.ToString());
            CoreWebList<FactorHeadingClass> objFactorHeadingList = objFactorHeading.fn_getFactorHeadingByID();
            if (objFactorHeadingList.Count > 0)
            {
                if (File.Exists(MapPath("~/files/Icons/" + objFactorHeadingList[0].strIcon)))
                {
                    File.Delete((MapPath("~/files/Icons/" + objFactorHeadingList[0].strIcon)));
                }
            }

			string strResponse = objFactorHeading.fn_deleteFactorHeading();
    ...
    protected void dv_Records_ItemInserting(object sender, DetailsViewInsertEventArgs e)
    {
        try
        {
			...
			FactorHeadingClass objFactorHeading = new FactorHeadingClass();
			objFactorHeading.strTitle = txtTitle.Text;

            if (fu_Icon.HasFile)
            {
                cls_common objCFC = new cls_common();
                string strRanFileName = objCFC.file_name(fu_Icon.FileName);
                string strDocPath = Server.MapPath("~/files/Icons/" + strRanFileName);
                fu_Icon.SaveAs(strDocPath);
                objFactorHeading.strIcon = strRanFileName;
            }

			string strResponse = objFactorHeading.fn_saveFactorHeading();
    ...
    protected void dv_Records_ItemUpdating(object sender, DetailsViewUpdateEventArgs e)
    {
        try
        {
			...
			FactorHeadingClass objFactorHeading = new FactorHeadingClass();
			objFactorHeading.iID = int.Parse(dv_Records.DataKey.Value.ToString());
			objFactorHeading.strTitle = txtTitle.Text;

            if (fu_Icon.HasFile)
            {
                FactorHeadingClass oFactorHeading = new FactorHeadingClass();
                oFactorHeading.iID = int.Parse(dv_Records.DataKey.Value.ToString());
                CoreWebList<FactorHeadingClass> oFactorHeadingList = oFactorHeading.fn_getFactorHeadingByID();
                if (oFactorHeadingList.Count > 0)
                {
                    if (File.Exists(MapPath("~/files/Icons/" + oFactorHeadingList[0].strIcon)))
                    {
                        File.Delete((MapPath("~/files/Icons/" + oFactorHeadingList[0].strIcon)));
                    }
                    cls_common objCFC = new cls_common();
                    string strRanFileName = objCFC.file_name(fu_Icon.FileName);
                    string strDocPath = Server.MapPath("~/files/Icons/" + strRanFileName);
            ...
            {
                objFactorHeading.strIcon = hf_Icon.Value;
            }

			string strResponse = objFactorHeading.fn_editFactorHeading();
    ...
    protected void btnSearch_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            FactorHeadingClass objFactorHeading = new FactorHeadingClass();
			objFactorHeading.strTitle = txtSearch.Text.Trim();
			CoreWebList<FactorHeadingClass> objFactorHeadingList = objFactorHeading.fn_getFactorHeadingByKeys();
			if (objFactorHeadingList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objFactorHeadingList;
				grd_Records.DataSource = objFactorHeadingList;
