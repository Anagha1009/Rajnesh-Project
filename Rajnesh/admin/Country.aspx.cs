...
public partial class admin_Country : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            BredCrumbs.InnerHtml = "<a class='link' href='" + VirtualPathUtility.ToAbsolute("~/admin/") + "'>Home</a> &nbsp; > &nbsp; &nbsp;Country";
    ...
    protected void fn_BindRecords()
    {
        try
        {
			...
			CoreWebList<CountryClass> objCountryList = objCountry.fn_getCountryList();
			if (objCountryList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objCountryList;
				grd_Records.DataSource = objCountryList;
			}
			else
			{
    ...
    protected void fn_BindDetails(int iRecordID)
    {
        ...
        {
            CountryClass objCountry = new CountryClass();
			objCountry.iID = iRecordID;
			CoreWebList<CountryClass> objCountryList = objCountry.fn_getCountryByID();
			if (objCountryList.Count > 0)
			{
				dv_Records.DataSource = objCountryList;
				dv_Records.DataBind();
			}
        }
        catch (Exception ex)
    ...
    protected void grd_Records_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        {
            CountryClass objCountry = new CountryClass();
			objCountry.iID = int.Parse(grd_Records.DataKeys[e.RowIndex].Value.ToString());
			CoreWebList<CountryClass> objCountryList = objCountry.fn_getCountryByID();
			if (objCountryList.Count > 0)
			{
				if (File.Exists(MapPath("~/files/Country/" + objCountryList[0].strPhoto)))
				{
					File.Delete((MapPath("~/files/Country/" + objCountryList[0].strPhoto)));
				}
			}

			string strResponse = objCountry.fn_deleteCountry();
    ...
    protected void dv_Records_ItemInserting(object sender, DetailsViewInsertEventArgs e)
    {
        try
        {
			...
			CountryClass objCountry = new CountryClass();
			objCountry.strTitle = txtTitle.Text;
            objCountry.strDetails = txtDetails.Text;

			if (fu_Photo.HasFile)
			{
				cls_common objCFC = new cls_common();
				string strRanFileName = objCFC.file_name(fu_Photo.FileName);
				string strDocPath = Server.MapPath("~/files/Country/" + strRanFileName);
				fu_Photo.SaveAs(strDocPath);
				objCountry.strPhoto = strRanFileName;
			}

			string strResponse = objCountry.fn_saveCountry();
    ...
    protected void dv_Records_ItemUpdating(object sender, DetailsViewUpdateEventArgs e)
    {
        try
        {
			...
			CountryClass objCountry = new CountryClass();
			objCountry.iID = int.Parse(dv_Records.DataKey.Value.ToString());
			objCountry.strTitle = txtTitle.Text;
            objCountry.strDetails = txtDetails.Text;

			if (fu_Photo.HasFile)
			{
				CountryClass oCountry = new CountryClass();
				oCountry.iID = int.Parse(dv_Records.DataKey.Value.ToString());
				CoreWebList<CountryClass> oCountryList = oCountry.fn_getCountryByID();
				if (oCountryList.Count > 0)
				{
					if (File.Exists(MapPath("~/files/Country/" + oCountryList[0].strPhoto)))
					{
						File.Delete((MapPath("~/files/Country/" + oCountryList[0].strPhoto)));
					}
					cls_common objCFC = new cls_common();
					string strRanFileName = objCFC.file_name(fu_Photo.FileName);
					string strDocPath = Server.MapPath("~/files/Country/" + strRanFileName);
			...
			{
				objCountry.strPhoto = hf_Photo.Value;
			}

			string strResponse = objCountry.fn_editCountry();
    ...
    protected void btnSearch_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            CountryClass objCountry = new CountryClass();
			objCountry.strTitle = txtSearch.Text.Trim();
			CoreWebList<CountryClass> objCountryList = objCountry.fn_getCountryByKeys();
			if (objCountryList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objCountryList;
				grd_Records.DataSource = objCountryList;
