...
public partial class admin_Banner : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            BredCrumbs.InnerHtml = "<a class='link' href='" + VirtualPathUtility.ToAbsolute("~/admin/") + "'>Home</a> &nbsp; > &nbsp; &nbsp;Banner";
    ...
    protected void fn_BindRecords()
    {
        try
        {
			...
			CoreWebList<BannerClass> objBannerList = objBanner.fn_getBannerList();
			if (objBannerList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objBannerList;
				grd_Records.DataSource = objBannerList;
			}
			else
			{
    ...
    protected void fn_BindDetails(int iRecordID)
    {
        ...
        {
            BannerClass objBanner = new BannerClass();
			objBanner.iID = iRecordID;
			CoreWebList<BannerClass> objBannerList = objBanner.fn_getBannerByID();
			if (objBannerList.Count > 0)
			{
				dv_Records.DataSource = objBannerList;
				dv_Records.DataBind();
			}
        }
        catch (Exception ex)
    ...
    protected void grd_Records_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        {
            BannerClass objBanner = new BannerClass();
			objBanner.iID = int.Parse(grd_Records.DataKeys[e.RowIndex].Value.ToString());
			CoreWebList<BannerClass> objBannerList = objBanner.fn_getBannerByID();
			if (objBannerList.Count > 0)
			{
				if (File.Exists(MapPath("~/files/Banner/" + objBannerList[0].strPhoto)))
				{
					File.Delete((MapPath("~/files/Banner/" + objBannerList[0].strPhoto)));
				}
			}

			string strResponse = objBanner.fn_deleteBanner();
    ...
    protected void dv_Records_ItemInserting(object sender, DetailsViewInsertEventArgs e)
    {
        try
        {
			...
			TextBox txtTargetUrl = (TextBox)dv_Records.FindControl("txtTargetUrl");
			FileUpload fu_Photo = (FileUpload)dv_Records.FindControl("fu_Photo");

			BannerClass objBanner = new BannerClass();
			objBanner.strTitle = txtTitle.Text;
			objBanner.strDetails = txtDetails.Text;
			objBanner.strTargetUrl = txtTargetUrl.Text;

			if (fu_Photo.HasFile)
			{
				cls_common objCFC = new cls_common();
				string strRanFileName = objCFC.file_name(fu_Photo.FileName);
				string strDocPath = Server.MapPath("~/files/Banner/" + strRanFileName);
				fu_Photo.SaveAs(strDocPath);
				objBanner.strPhoto = strRanFileName;
			}

			string strResponse = objBanner.fn_saveBanner();
    ...
    protected void dv_Records_ItemUpdating(object sender, DetailsViewUpdateEventArgs e)
    {
        try
        {
			...
			BannerClass objBanner = new BannerClass();
			objBanner.iID = int.Parse(dv_Records.DataKey.Value.ToString());
			objBanner.strTitle = txtTitle.Text;
			objBanner.strDetails = txtDetails.Text;
			objBanner.strTargetUrl = txtTargetUrl.Text;
			...
			if (fu_Photo.HasFile)
			{
				BannerClass oBanner = new BannerClass();
				oBanner.iID = int.Parse(dv_Records.DataKey.Value.ToString());
				CoreWebList<BannerClass> oBannerList = oBanner.fn_getBannerByID();
				if (oBannerList.Count > 0)
				{
					if (File.Exists(MapPath("~/files/Banner/" + oBannerList[0].strPhoto)))
					{
						File.Delete((MapPath("~/files/Banner/" + oBannerList[0].strPhoto)));
					}
					cls_common objCFC = new cls_common();
					string strRanFileName = objCFC.file_name(fu_Photo.FileName);
					string strDocPath = Server.MapPath("~/files/Banner/" + strRanFileName);
			...
			{
				objBanner.strPhoto = hf_Photo.Value;
			}

			string strResponse = objBanner.fn_editBanner();
    ...
    protected void btnSearch_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            BannerClass objBanner = new BannerClass();
			objBanner.strTitle = txtSearch.Text.Trim();
			CoreWebList<BannerClass> objBannerList = objBanner.fn_getBannerByKeys();
			if (objBannerList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objBannerList;
				grd_Records.DataSource = objBannerList;
