...
public partial class admin_Factor : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
			...
			if (Request.QueryString["HeadingID"] != null)
			{
				iHeadingID = int.Parse(Request.QueryString["HeadingID"].ToString());
			}
			else
			{
                Response.Redirect(VirtualPathUtility.ToAbsolute("~/admin/FactorHeading.aspx"), true);
			}

            HtmlGenericControl BredCrumbs = (HtmlGenericControl)Master.FindControl("BredCrumbs");
            BredCrumbs.InnerHtml = "<a class='link' href='" + VirtualPathUtility.ToAbsolute("~/admin/") + "'>Home</a> &nbsp; > &nbsp; &nbsp;<a class='link' href='" + VirtualPathUtility.ToAbsolute("~/admin/FactorHeading.aspx") + "'>Factor Headings</a> &nbsp; > &nbsp; &nbsp;Factor";
    ...
    protected void fn_BindRecords()
    {
        try
        {
			...
			objFactor.iHeadingID = iHeadingID;
			CoreWebList<FactorClass> objFactorList = objFactor.fn_getFactorByHeadingID();
			if (objFactorList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objFactorList;
				grd_Records.DataSource = objFactorList;
			}
			else
			{
    ...
    protected void fn_BindDetails(int iRecordID)
    {
        ...
        {
            FactorClass objFactor = new FactorClass();
			objFactor.iID = iRecordID;
			CoreWebList<FactorClass> objFactorList = objFactor.fn_getFactorByID();
			if (objFactorList.Count > 0)
			{
				dv_Records.DataSource = objFactorList;
				dv_Records.DataBind();
			}
        }
        catch (Exception ex)
    ...
    protected void grd_Records_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        {
            FactorClass objFactor = new FactorClass();
			objFactor.iID = int.Parse(grd_Records.DataKeys[e.RowIndex].Value.ToString());

			string strResponse = objFactor.fn_deleteFactor();
    ...
    protected void dv_Records_ItemInserting(object sender, DetailsViewInsertEventArgs e)
    {
        try
        {
			...
			FactorClass objFactor = new FactorClass();
			objFactor.iHeadingID = iHeadingID;
			objFactor.strTitle = txtTitle.Text;

			string strResponse = objFactor.fn_saveFactor();
    ...
    protected void dv_Records_ItemUpdating(object sender, DetailsViewUpdateEventArgs e)
    {
        try
        {
			...
			FactorClass objFactor = new FactorClass();
			objFactor.iID = int.Parse(dv_Records.DataKey.Value.ToString());
			objFactor.strTitle = txtTitle.Text;

			string strResponse = objFactor.fn_editFactor();
    ...
    protected void btnSearch_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            FactorClass objFactor = new FactorClass();
			objFactor.strTitle = txtSearch.Text.Trim();
			CoreWebList<FactorClass> objFactorList = objFactor.fn_getFactorByKeys();
			if (objFactorList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objFactorList;
				grd_Records.DataSource = objFactorList;
