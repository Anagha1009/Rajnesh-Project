...
public partial class admin_PagePhoto : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
			...
			{
				iPageID = int.Parse(Request.QueryString["PageID"].ToString());
			}
			else
			{
				Response.Redirect(VirtualPathUtility.ToAbsolute("~/admin/Page.aspx"), true);
			}

            HtmlGenericControl BredCrumbs = (HtmlGenericControl)Master.FindControl("BredCrumbs");
            BredCrumbs.InnerHtml = "<a class='link' href='" + VirtualPathUtility.ToAbsolute("~/admin/") + "'>Home</a> &nbsp; > &nbsp; &nbsp;PagePhoto";
    ...
    protected void fn_BindRecords()
    {
        try
        {
            PagePhotoClass objPagePhoto = new PagePhotoClass();
			objPagePhoto.iPageID = iPageID;
			CoreWebList<PagePhotoClass> objPagePhotoList = objPagePhoto.fn_getPagePhotoByPageID();
			if (objPagePhotoList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objPagePhotoList;
				grd_Records.DataSource = objPagePhotoList;
			}
			else
			{
    ...
    protected void fn_BindDetails(int iRecordID)
    {
        ...
        {
            PagePhotoClass objPagePhoto = new PagePhotoClass();
			objPagePhoto.iID = iRecordID;
			CoreWebList<PagePhotoClass> objPagePhotoList = objPagePhoto.fn_getPagePhotoByID();
			if (objPagePhotoList.Count > 0)
			{
				dv_Records.DataSource = objPagePhotoList;
				dv_Records.DataBind();
			}
        }
        catch (Exception ex)
    ...
    protected void grd_Records_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        {
            PagePhotoClass objPagePhoto = new PagePhotoClass();
			objPagePhoto.iID = int.Parse(grd_Records.DataKeys[e.RowIndex].Value.ToString());
			CoreWebList<PagePhotoClass> objPagePhotoList = objPagePhoto.fn_getPagePhotoByID();
			if (objPagePhotoList.Count > 0)
			{
				if (File.Exists(MapPath("~/files/PagePhoto/" + objPagePhotoList[0].strPhoto)))
				{
					File.Delete((MapPath("~/files/PagePhoto/" + objPagePhotoList[0].strPhoto)));
				}
			}

			string strResponse = objPagePhoto.fn_deletePagePhoto();
    ...
    protected void dv_Records_ItemInserting(object sender, DetailsViewInsertEventArgs e)
    {
        try
        {
			...
			PagePhotoClass objPagePhoto = new PagePhotoClass();
			objPagePhoto.iPageID = iPageID;
			objPagePhoto.strTitle = txtTitle.Text;

			if (fu_Photo.HasFile)
			{
				cls_common objCFC = new cls_common();
				string strRanFileName = objCFC.file_name(fu_Photo.FileName);
				string strDocPath = Server.MapPath("~/files/PagePhoto/" + strRanFileName);
				fu_Photo.SaveAs(strDocPath);
				objPagePhoto.strPhoto = strRanFileName;
			}

			string strResponse = objPagePhoto.fn_savePagePhoto();
    ...
    protected void dv_Records_ItemUpdating(object sender, DetailsViewUpdateEventArgs e)
    {
        try
        {
			...
			PagePhotoClass objPagePhoto = new PagePhotoClass();
			objPagePhoto.iID = int.Parse(dv_Records.DataKey.Value.ToString());
			objPagePhoto.strTitle = txtTitle.Text;

			if (fu_Photo.HasFile)
			{
				PagePhotoClass oPagePhoto = new PagePhotoClass();
				oPagePhoto.iID = int.Parse(dv_Records.DataKey.Value.ToString());
				CoreWebList<PagePhotoClass> oPagePhotoList = oPagePhoto.fn_getPagePhotoByID();
				if (oPagePhotoList.Count > 0)
				{
					if (File.Exists(MapPath("~/files/PagePhoto/" + oPagePhotoList[0].strPhoto)))
					{
						File.Delete((MapPath("~/files/PagePhoto/" + oPagePhotoList[0].strPhoto)));
					}
					cls_common objCFC = new cls_common();
					string strRanFileName = objCFC.file_name(fu_Photo.FileName);
					string strDocPath = Server.MapPath("~/files/PagePhoto/" + strRanFileName);
			...
			{
				objPagePhoto.strPhoto = hf_Photo.Value;
			}

			string strResponse = objPagePhoto.fn_editPagePhoto();
    ...
    protected void btnSearch_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            PagePhotoClass objPagePhoto = new PagePhotoClass();
			objPagePhoto.strTitle = txtSearch.Text.Trim();
			CoreWebList<PagePhotoClass> objPagePhotoList = objPagePhoto.fn_getPagePhotoByKeys();
			if (objPagePhotoList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objPagePhotoList;
				grd_Records.DataSource = objPagePhotoList;
