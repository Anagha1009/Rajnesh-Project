...
public partial class admin_Article : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
			...
			{
				iCountryID = int.Parse(Request.QueryString["CountryID"].ToString());
			}
			else
			{
            ...
            BredCrumbs.InnerHtml = "<a class='link' href='" + VirtualPathUtility.ToAbsolute("~/admin/") + "'>Home</a> &nbsp; > &nbsp; &nbsp;Article";
    ...
    protected void fn_BindRecords()
    {
        try
        {
			...
			objArticle.iCountryID = iCountryID;
			CoreWebList<ArticleClass> objArticleList = objArticle.fn_getArticleByCountryID();
			if (objArticleList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objArticleList;
				grd_Records.DataSource = objArticleList;
			}
			else
			{
    ...
    protected void fn_BindDetails(int iRecordID)
    {
        ...
        {
            ArticleClass objArticle = new ArticleClass();
			objArticle.iID = iRecordID;
			CoreWebList<ArticleClass> objArticleList = objArticle.fn_getArticleByID();
			if (objArticleList.Count > 0)
			{
				dv_Records.DataSource = objArticleList;
				dv_Records.DataBind();
			}
        }
        catch (Exception ex)
    ...
    protected void grd_Records_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        {
            ArticleClass objArticle = new ArticleClass();
			objArticle.iID = int.Parse(grd_Records.DataKeys[e.RowIndex].Value.ToString());
			CoreWebList<ArticleClass> objArticleList = objArticle.fn_getArticleByID();
			if (objArticleList.Count > 0)
			{
				if (File.Exists(MapPath("~/files/Article/" + objArticleList[0].strPhoto)))
				{
					File.Delete((MapPath("~/files/Article/" + objArticleList[0].strPhoto)));
				}
			}

			string strResponse = objArticle.fn_deleteArticle();
    ...
    protected void dv_Records_ItemInserting(object sender, DetailsViewInsertEventArgs e)
    {
        try
        {
			...
			ArticleClass objArticle = new ArticleClass();
			objArticle.iCountryID = iCountryID;
			objArticle.strTitle = txtTitle.Text;
			objArticle.strDetails = txtDetails.Text;

			if (fu_Photo.HasFile)
			{
				cls_common objCFC = new cls_common();
				string strRanFileName = objCFC.file_name(fu_Photo.FileName);
				string strDocPath = Server.MapPath("~/files/Article/" + strRanFileName);
				fu_Photo.SaveAs(strDocPath);
				objArticle.strPhoto = strRanFileName;
			}

			string strResponse = objArticle.fn_saveArticle();
    ...
    protected void dv_Records_ItemUpdating(object sender, DetailsViewUpdateEventArgs e)
    {
        try
        {
			...
			ArticleClass objArticle = new ArticleClass();
			objArticle.iID = int.Parse(dv_Records.DataKey.Value.ToString());
			objArticle.strTitle = txtTitle.Text;
			objArticle.strDetails = txtDetails.Text;

			if (fu_Photo.HasFile)
			{
				ArticleClass oArticle = new ArticleClass();
				oArticle.iID = int.Parse(dv_Records.DataKey.Value.ToString());
				CoreWebList<ArticleClass> oArticleList = oArticle.fn_getArticleByID();
				if (oArticleList.Count > 0)
				{
					if (File.Exists(MapPath("~/files/Article/" + oArticleList[0].strPhoto)))
					{
						File.Delete((MapPath("~/files/Article/" + oArticleList[0].strPhoto)));
					}
					cls_common objCFC = new cls_common();
					string strRanFileName = objCFC.file_name(fu_Photo.FileName);
					string strDocPath = Server.MapPath("~/files/Article/" + strRanFileName);
			...
			{
				objArticle.strPhoto = hf_Photo.Value;
			}

			string strResponse = objArticle.fn_editArticle();
    ...
    protected void btnSearch_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            ArticleClass objArticle = new ArticleClass();
			objArticle.strTitle = txtSearch.Text.Trim();
			CoreWebList<ArticleClass> objArticleList = objArticle.fn_getArticleByKeys();
			if (objArticleList.Count > 0)
			{
				ViewState["dtRecords"] = (DataTable)objArticleList;
				grd_Records.DataSource = objArticleList;
